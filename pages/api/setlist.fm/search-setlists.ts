...
export default async function handler(req: NextApiRequest, res: NextApiResponse) {
    ...
    async function fetchSetlist(retries: number = 0): Promise<Response> {
        const url = `https://api.setlist.fm/rest/1.0/search/setlists?artistName=${artistName}&p=1&sort=recency`;
    ...
    try {
        ...
        if (!data.setlist || data.setlist.length === 0) {
            res.status(404).json({
                error: `No setlist with songs found for artist: ${artistName}`
            });
            return;
        }

        const setlist = data.setlist.find(
            (setlist) =>
                setlist.artist.name.toLowerCase() === artistName.toString().toLowerCase() &&
                setlist.sets.set.length > 0 &&
                setlist.sets.set.some((s) => s.song && s.song.length > 0)
        );
